import { IOptions, ISvg } from "./custom";
import { svgToString } from "../ui-src/utils/svgToString.util";
import JSZip from "jszip";

export const saveAsFiles = (svgs: ISvg[], options: IOptions) => {
  const extension = `.${options.typescript.value ? "t" : "j"}sx`;

  if (svgs.length === 1) {
    const svg = svgs[0];
    const blobURL = window.URL.createObjectURL(
      new Blob([svgToString(svg)])
    );
